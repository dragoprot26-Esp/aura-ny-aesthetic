/**
 * cloud.ts - Capa de nube (Supabase) para Aura NY.
 ...
 * propias de esta app (aura_*). Prefijo de datos: AURA.
 */
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('aura_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('aura_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// -- Sync de datos del local (tabla aura_backups, RLS por membresia) --
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/aura_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/aura_backups`, {
      method: 'POST',
      headers: {
...
// -- Publico (catalogo) y alta de pedido/turno --
export async function auraPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('aura_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function auraAgregarPedido(codigo: string, pedido: any): Promise<void> {
  try { await rpc('aura_agregar_pedido', { p_codigo: codigo, p_pedido: pedido }, false); }
  catch (e) { /* noop */ }
}
